  ...
  return (
    <div className="m-52 flex-col flex-wrap w-1/4 shadow-2xl cursor-pointer hover:scale-110 duration-300">
      <div className="p-2">
        ...
        <div>
          <div className="p-7">{data[2]?.[0]?.pitch}</div>

          <p className="text-xl font-bold pl-3">Student will achieve:</p>
          <div className="p-3">{data[2]?.[0]?.curriculum_outcomes}</div>
        </div>
        ...
        <div className="flex justify-around text-xl font-semibold">
          RS {data[2]?.[0]?.discounted_price}
          <span>
            Rs{" "}
            {Math.ceil(
              data[2]?.[0]?.discounted_price / data[2]?.[0]?.num_classes
            )}{" "}
            Per class{" "}
          </span>
        </div>
        <div className="pl-9 text-gray font-semibold text-lg line-through">
          Rs {data[2]?.[0]?.original_price}
        </div>
        ...
        <div className="flex justify-center items-center space-x-11 py-4 bg-color1dark h-5">
          ...
          <div>{data[2]?.[0]?.games_count} Games</div>
          <div>{data[2]?.[0]?.certificate_count} Certificate</div>
        </div>
